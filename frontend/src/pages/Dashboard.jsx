...
import { TrendingUp, TrendingDown, Target, Building2, Download } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-t-8 border-t-green-500 overflow-hidden">
          <div className="flex justify-between items-start">
            <div className="min-w-0 pr-4">
              <p className="text-slate-500 font-bold text-lg xl:text-xl uppercase tracking-wider truncate" title="Target Opsen BBNKB">Realisasi Opsen BBNKB</p>
              <h3 className="text-3xl xl:text-4xl font-black text-slate-800 mt-2 truncate">{formatRupiah(realisasiBbnkb)}</h3>
            </div>
          ...
          <div className="mt-6">
            <div className="w-full bg-slate-200 rounded-full h-4">
              <div className="bg-green-500 h-4 rounded-full" style={{ width: `${Math.min(bbnkbPercent, 100)}%` }}></div>
            </div>
            <p className="mt-3 text-lg font-semibold text-slate-600">Terpenuhi: {bbnkbPercent.toFixed(1)}% dari {formatRupiah(targetBbnkb)} {bbnkbPercent < 50 && <span className="text-red-500 font-bold">(Warning)</span>}</p>
          </div>
        </div>
        ...
        <div className="bg-primary-800 p-6 rounded-2xl shadow-lg border border-primary-900 border-t-8 border-t-yellow-400 text-white overflow-hidden">
          <div className="flex justify-between items-start">
            <div className="min-w-0 pr-4">
              <p className="text-primary-200 font-bold text-lg xl:text-xl uppercase tracking-wider truncate" title="Estimasi Sisa Target Keseluruhan">Sisa Target Total</p>
              <h3 className="text-4xl xl:text-4xl font-black mt-3 text-yellow-400 drop-shadow-md truncate">{formatRupiah(Math.max(sisaTarget, 0))}</h3>
            </div>
          ...
          <div className="mt-6">
            <p className="text-lg font-medium text-primary-100 truncate">Evaluasi akhir tahun</p>
          </div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto max-h-[400px] overflow-y-auto border border-slate-100 rounded-xl relative shadow-inner">
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-slate-50 z-10 shadow-sm border-b-2 border-slate-200">
                <tr>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">Kecamatan</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">PKB Pokok</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">Opsen PKB</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">BBNKB Pokok</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">Opsen BBNKB</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">Total Realisasi Opsen</th>
                  <th className="p-4 text-base font-bold text-slate-700 bg-slate-50 whitespace-nowrap">Presentase</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {loadingKec ? (
                  <tr><td colSpan="7" className="p-8 text-center text-slate-500 text-lg font-bold">Memuat Data Kecamatan..</td></tr>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-primary-50 transition-colors">
                      ...
                      <td className="p-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold ${percent >= 100 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                          }`}>
                          {percent.toFixed(1)}%
                        </span>
                      </td>
