  ...
  return (
    <div className="min-h-screen bg-primary-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        <img src={logoProvinsi} alt="Logo Provinsi" className="h-28 w-auto mb-5 drop-shadow-lg" />
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-10 px-6 shadow-2xl rounded-2xl sm:px-10 border border-slate-100">
          <form className="space-y-8" onSubmit={handleLogin}>
            <div>
              <label htmlFor="username" className="block text-lg font-bold text-slate-700 mb-2">
                Username Akun
              </label>
              <input
                id="username"
            ...
            <div>
              <label htmlFor="password" className="block text-lg font-bold text-slate-700 mb-2">
                Password
              </label>
              <input
                id="password"
            ...
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl text-blue-800 text-[15px]">
              <strong>Demo Hint:</strong> Gunakan username <code>admin</code> atau <code>kadin</code> untuk melihat beranda.
            </div>
            ...
            {loginMutation.isError && (
              <div className="text-red-600 font-semibold text-center mt-4 p-3 bg-red-50 rounded-xl">
                 Error: {loginMutation.error.response?.data?.message || 'Gagal tersambung ke server'}
              </div>
            )}
          </form>
        </div>
