...
export default function MainLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden relative">
      ...
      {/* Sidebar */}
      <aside
        className={`w-72 bg-primary-800 text-white flex flex-col shadow-xl z-30 absolute md:relative h-full transition-transform duration-300 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        <div className="p-6 bg-primary-900 border-b border-primary-700 flex items-center gap-4">
          ...
          />
          <div>
            <p className="text-primary-200 text-xs font-bold uppercase tracking-wider mb-0.5">
              UPT PPD Magetan
            </p>
              ...
              return (
                <li key={item.path}>
                  <Link
                    to={item.path}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg text-[17px] font-medium transition-colors ${
                      isActive
                      ...
                      return (
                        <li key={subItem.path}>
                          <Link
                            to={subItem.path}
                            onClick={() => setIsMobileMenuOpen(false)}
                            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-[15px] font-medium transition-colors ${
                              isSubActive
        ...
        <header className="bg-white border-b border-slate-200 h-20 px-4 md:px-8 flex items-center justify-between shadow-sm relative z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-slate-800 truncate max-w-[200px] md:max-w-none">
              {navItems
                .concat(masterDataItems)
                .find((i) => location.pathname.startsWith(i.path))?.name ||
                "SIOPTIMA"}
            </h2>
          </div>
